...
\begin{definition}{4.1.3}[Joint probability mass function]
  ...
  joint pmf of the vector $(X, Y)$ rather than some other vector, the notation $f_{X,Y}(x, y)$
  will be used.
\end{definition}
...
\begin{definition}{4.6.2}[Multinomial Distribution]
  Let $n$ and $m$ be positive integers and let $p_1, \ldots, p_n$ be numbers satisfying
  $0 \leq p_i \leq 1, i = 1, \ldots, n$ and $\sum_{i=1}^n p_i = 1$.
  Then the random vector $(X_1, \ldots, X_n)$ has a
  \textit{multinomial distribution with $m$ trials and cell probabilities $p_1, \ldots, p_n$}
  if the joint pmf of $(X_1, \ldots, X_n)$ is
  \begin{equation*}
    f(x_1, \ldots, x_n)
    = \frac{m!}{x_1! \ldots x_n!} p_1^{x_1} \ldots p_n^{x_n}
    = m! \prod_{i=1}^n \frac{p_i^{x_i}}{x_i!}
  \end{equation*}
  on the set of $(x_1, \ldots, x_n)$ such that each $x_i$ is a nonnegative integer
  and $\sum_{i = 1}^n x_i = m$.
\end{definition}

\begin{theorem}{4.6.2}[Multinomial Theorem]
  Let $m$ and $n$ be positive integers.
  Let $\mathcal{A}$ be the set of vectors $\vec{x} = (x_1, \ldots, x_n)$
  such that each $x_i$ is a nonnegative integer
  and $\sum \limits_{i=1}^{n} x_i = m$. Then, for any real numbers $p_1, \ldots, p_n$,
  \begin{equation*}
    (p_1 + \ldots + p_n)^m
    =
    \sum_{x \in \mathcal{A}} \frac{m!}{x_1! \ldots x_n!} p_1^{x_1} \ldots p_n^{x_n}.
  \end{equation*}
\end{theorem}

\begin{definition}{4.6.5}
  Let $\vec{X}_1, \ldots, \vec{X}_n$ be random vectors with joint pdf or pmf
  $f(\vec{x}_1, \ldots, \vec{x}_n)$. Let $f_{\vec{X}_i}(\vec{x}_i)$ denote the
  marginal pdf or pmf of $\vec{X}_i$. Then $\vec{X_1}, \ldots, \vec{X_n}$
  are called \textit{mutually independent random vectors} if, for every
  $(\vec{x}_1, \ldots, \vec{x}_n)$,
  \begin{equation*}
    f(\vec{x}_1, \ldots, \vec{x}_n) = \prod_{i = 1}^{n} f_{\vec{X}_i}(\vec{x}_i).
  \end{equation*}
  If the $X_i$s are all one-dimensional, then $X_1, \ldots, X_n$ are called
...
\begin{theorem}{4.6.12}[]
  Let $\vec{X}_1, \ldots, \vec{X}_n$ be independent random vectors. Let $g_i(\vec{x}_i)$
  be a function only of $\vec{x}_i, i = 1, \ldots, n$. Then the random variables
  $U_i = g_i(\vec{X}_i), i = 1, \ldots, n$, are mutually independent.
\end{theorem}

\begin{theorem}{4.6.13}[]
  Let $(X_1, \ldots, X_n)$ be a random vector with pdf $f(x_1, \ldots, x_n)$.
  Consider a new random vector $(U_1, \ldots, U_n)$, defined by
  $U_j = g_j(X_1, \ldots, X_n), j = 1, \ldots, n$.
  Denote the $i$th inverse by $x_1 = h_{1i}(u_1, \ldots, u_n)$.
  Then
  \begin{align*}
    & f_{\vec{U}}(u_1, \ldots, u_n)
    \\
    & ~~~~ = \sum_{i = 1}^{k} f_{\vec{X}}(h_{1i}(u_1, \ldots, u_n), \ldots, h_{ni}(u_1, \ldots, u_n)) |J_i|,
  \end{align*}
  where
  \begin{equation*}
...
\begin{definition}{5.1.1}[Random sample]
  The random variables $X_1, \ldots, X_n$ are called a \textit{random sample of size $n$ from the population $f(x)$} 
  if $X_1, \ldots, X_n$ are mutually independent random variables and the marginal pdf or pmf of each $X_i$ is the same function $f(x)$.
  Alternatively, $X_1, \ldots, X_n$ are called \textit{independent and identically distributed random variables with pdf or pmf $f(x)$.} 
  This is commonly abbreviated to iid random variables.
\end{definition}

\begin{definition}{5.2.1}[Statistics]
  Let $X_1, \ldots, X_n$ be a random sample of size $n$ from a population and let $T(x_1, \ldots, x_n)$ be a real-valued or vector-valued
  function whose domain includes the sample space of $(X_1, \ldots, X_n)$. Then the random variable or random vector
  $Y = T(X_1, \ldots, X_n)$ is called a \textit{statistic}. The probability distribution of a statistic $Y$ is called the
...
\begin{theorem}{5.2.4}[Statistical properties]
  Let $x_1, \ldots, x_n$ be any numbers and $\bar{x} = (x_1 + \ldots + x_n)/n$.
  Then
  \begin{enumerate}[label=\alph*.]
...
\begin{lemma}{5.2.5}
  Let $X_1, \ldots, X_n$ be a random sample from a population and let $g(x)$ be a function such that
...
\begin{theorem}{5.2.6}
  Let $X_1, \ldots, X_n$ be a random sample from a population with mean $\mu$ and variance
  $\sigma^2 < \infty$. Then
  \begin{enumerate}[label=\alph*.]
    \item $\E{\bar{X}} = \mu$,
...
\begin{theorem}{5.2.7}[MGF of sample mean]
  Let $X_1, \ldots, X_n$ be a random sample from a population with mgf $M_X(t)$.
...
\begin{theorem}{5.2.11}
  ...
  \end{equation*}
  is a member of the exponential family. Define statistics $T_1, \ldots, T_k$ by
  \begin{equation*}
    T_i(X_1, \ldots, X_n) = \sum_{j = 1}^{n} t_i(X_j), ~~~~~ i = 1,\ldots,k.
  \end{equation*}
  If the set $\{(w_1(\theta), \ldots, w_k(\theta)), \theta \in \Theta \}$ contains an open subset
  of $\mathcal{R}^k$, then the distribution of $(T_1, \ldots, T_k)$ is an exponential family
  of the form
  \begin{align*}
    f_T(u_1, \ldots, u_k | \theta)
    &=
    \\
    H(u_1, \ldots, u_k)
    \left[c(\theta)\right]^n &
...
\begin{theorem}{5.3.1}
  Let $X_1, \ldots, X_n$ be a random sample from a $\mathcal{N}(\mu, \sigma^{2})$
...
\begin{lemma}{5.3.2}[Facts about chi squared random variables]
  ...
  \begin{enumerate}[label=\alph*.]
    ...
    \item If $X_1, \ldots, X_n$ are independent and $X_i \sim \chi_{p_i}^2$, then
      $X_1 + \ldots + X_n \sim \chi_{p_1 + \ldots + p_n}^2$;
...
\begin{lemma}{5.3.3}
  Let $X_j \sim \mathcal{N}(\mu_j, \sigma_j^2)$, $j = 1, \ldots, n$, independent.
  For constants $a_{ij}$ and $b_{rj}$, $(j = 1, \ldots, n; i = 1, \ldots, k; r = 1, \ldots, m)$,
  ...
  \begin{enumerate}[label=\alph*.]
    ...
    \item The random vectors $(U_1, \ldots, U_k)$ and $(V_1, \ldots, V_m)$ are independent if only if
      $U_i$ is independent of $V_r$ for all pairs $i, r$ $(i = 1, \ldots, k; r = 1, \ldots, m)$.
  \end{enumerate}
\end{lemma}

\begin{definition}{5.3.4}[Student's t distribution]
  Let $X_1$, \ldots, $X_n$ be a random sample from a $\mathcal{N}(\mu, \sigma^2)$ distribution.
...
\begin{definition}{5.3.6}[Snedecor's F distribution]
  Let $X_1$, \ldots, $X_n$ be a random sample from a $\mathcal{N}(\mu_X, \sigma_X^2)$ population,
  and let $Y_1, \ldots, Y_m$ be a random sample from an independent $\mathcal{N}(\mu_Y, \sigma_Y^2)$
  population.
...
\begin{definition}{5.5.1}[Convergence in probability]
  A sequence of random variables, $X_1, X_2, \ldots$, \textit{converges in probability}
...
\begin{theorem}{5.5.2}[Weak Law of Large Numbers]
  Let $X_1, X_2, \ldots$ be iid random variables with $\E{X_i} = \mu$ and $\Var{X_i} = \sigma^2 < \infty$.
...
\begin{theorem}{5.5.4}
  \label{theorem:5.5.4}
  Suppose that $X_1, X_2, \ldots$ converges in probability to a random variable $X$
  and that $h$ is a continuous function. Then $h(X_1), h(X_2), \ldots$ converges
...
\begin{definition}{5.5.10}[Convergence in distribution]
  A sequence of random variables, $X_1, X_2, \ldots$, \textit{converges in distribution} 
...
\begin{theorem}{5.5.12}
  If the sequence of random variables, $X_1, X_2, \ldots$, converges in probability
  to a random variable $X$, the sequence also converges in distribution to $X$.
\end{theorem}

\begin{theorem}{5.5.13}
  The sequence of random variables, $X_1, X_2, \ldots$, converges in probability to a
...
\begin{theorem}{5.5.14}[Central Limit Theorem]
  $~$\\ Let $X_1, X_2, \ldots$ be a sequence of iid random variables whose mgfs exist in a neighborhood of $0$
...
\begin{theorem}{5.5.15}[Stronger form of the Central Limit Theorem]
  Let $X_1, X_2, \ldots$ be a sequence of iid random variables with $\E{X_i} = \mu$ and
...
\begin{theorem}{5.5.28}[Multivariate Delta Method]
  Let $\vec{X}_1, \ldots, \vec{X}_n$ be a random sample with $\E{X_{ij}} = \mu_i$ and $\Cov{X_{ik}}{X_{jk}} = \sigma_{ij}$.
  For a given function $g$ with continuous first partial derivatives and a specific value of $\vec{\mu} = (\mu_1, \ldots, \mu_p)$
  for which $\tau = \sum \sum \sigma_{ij} \pd{g(\vec{\mu})}{\mu_i} \cdot \pd{g(\vec{\mu})}{\mu_j} > 0$,
  \begin{equation*}
    \sqrt{n} \left[ g\left( \bar{X}_1, \ldots, \bar{X}_s \right) - g(\mu_1, \ldots, \mu_p) \right] \rightarrow \mathcal{N}(0, \tau^2)
  \end{equation*}
  in distribution.
\end{theorem}
...
\begin{definition}{6.2.0}[Sufficiency Principle]
  If $T(\vec{X})$ is a sufficient statistic for $\theta$, then any inference about $\theta$ should depend on the sample
...
\begin{theorem}{6.2.10}
  Let $X_1, \ldots, X_n$ be iid observations from a pdf or pmf $f(x|\theta)$ that belongs to an exponential family
  given by
  \begin{equation*}
    f(x|\vec{\theta}) = h(x)c(\vec{\theta}) \exp\left( \sum_{i = 1}^{k} w_i(\vec{\theta}) t_i(x) \right),
  \end{equation*}
  where $\vec{\theta} = (\theta_1, \theta_2, \ldots, \theta_d), d \leq k$. Then
  \begin{equation*}
    T(\vec{X}) = \left(
      \sum_{j = 1}^{n} t_1(X_j), \ldots, \sum_{j = 1}^{n} t_k(X_j)
    \right)
  \end{equation*}
...
\begin{theorem}{6.2.13}
  ...
  $\theta$ if and only if $T(\vec{x}) = T(\vec{y})$. Then $T(\vec{X})$ is a minimal
...
\begin{theorem}{6.2.25}[Complete Statistic in the Exponential Family]
  Let $X_1, \ldots, X_n$ be iid observations from an exponential family with pdf or pmf of the
  form
  \begin{equation*}
    ...
    \right)
  \end{equation*}
  where $\vec{\theta} = (\theta_1, \theta_2, \ldots, \theta_k)$. Then the statistic
  \begin{equation*}
    T(\vec{X}) = \left(
      \sum_{i = 1}^{n} t_1(X_i), \sum_{i = 1}^{n} t_2(X_i), \ldots, \sum_{i = 1}^{n} t_k(X_i)
    \right)
  \end{equation*}
...
\begin{definition}{7.1.1}[Point estimator]
  A \textit{point estimator} is any function $W(X_1, \ldots, X_n)$ of a sample;
...
\begin{definition}{7.2.0}[Method of moments]
  ...
  \begin{align*}
    ...
    \mu_j' &= \E{X^j}.
  \end{align*}
  The population moment $\mu_j'$ will typically be a function of $\theta_1, \ldots, \theta_k$,
  say $\mu_j'(\theta_1, \ldots, \theta_k)$.
  The method of moments involves solving the following system of equations for
  $(\theta_1, \ldots, \theta_k)$.
  \begin{equation*}
    m_j = \mu_j'(\theta_1, \ldots, \theta_k),~~~~j = 1, \ldots, k
  \end{equation*}
\end{definition}
...
\begin{theorem}{7.3.9}[Cramér-Rao Inequality]
  Let \\ $X_1, \ldots, X_n$ be a sample with pdf $f(\vec{x} | \theta)$, and let
  $W(\vec{X}) = W(X_1, \ldots, X_n)$ be any estimator satisfying
  \begin{equation*}
...
\begin{corollary}{7.3.10}[Cramér-Rao Inequality, iid case]
  ...
  if $X_1, \ldots, X_n$ are iid with pdf $f(x|\theta)$, then
  \begin{equation*}
    \mathrm{Var}_{\theta} \left( W(\vec{X}) \right)
    \geq
    \frac{
...
\begin{corollary}{7.3.15}[Attainment]
  Let $X_1, \ldots, X_n$ be iid $f(x|\theta)$, where $f(x|\theta)$ satisfies the conditions
  of the Cramér-Rao Theorem. Let $L(\theta|\vec{x}) = \prod_{i=1}^{n} f(x_i | \theta)$
  denote the likelihood function. If $W(\vec{X}) = W(X_1, \ldots, X_n)$ is any unbiased
...
\begin{definition}{8.1.2}[Null hypothesis]
  ...
  They are denoted by $H_0$ and $H_1$, respectively.
\end{definition}
...
\begin{theorem}{8.3.12}[Neyman-Pearson Lemma]
  ...
  Consider testing $H_0: \theta = \theta_0$ versus $H_1: \theta = \theta_1$, where the pdf or pmf
...
\begin{definition}{9.1.1}[Interval estimate]
  ...
  $L(x_1, \ldots, x_n)$ and $U(x_1, \ldots, x_n)$, of a sample that satisfy $L(\vec{x}) \leq U(\vec{x})$
...
\begin{definition}{9.1.4}[Coverage probability]
  ...
  parameter, $\theta$. In symbols, it is denoted by either $P_{\theta}(\theta \in [L(\vec{X}), U(\vec{X})])$
...
\begin{definition}{9.2.6}[Pivotal quantity]
  A random variable $Q(\vec{X}, \theta) = Q(X_1, \ldots, X_n, \theta)$ is a \textit{pivotal quantity} (or \textit{pivot})
...
\begin{theorem}{9.2.12}[Pivoting a continous cdf]
  Let $T$ be a statistic with continuous cdf $F_T(t | \theta)$. Let $\alpha_1 + \alpha_2 = \alpha$ with $0 < \alpha < 1$
...
\begin{theorem}{9.2.14}[Pivoting a discrete cdf]
  Let $T$ be a discrete statistic with cdf $F_T(t | \theta) = P(T \leq t | \theta)$. Let $\alpha_1 + \alpha_2 = \alpha$
  ...
  \begin{enumerate}[label=\roman*.]
    \item If $F_T(t | \theta)$ is a decreasing function of $\theta$ for each $t$, define $\theta_L(t)$ and $\theta_U(t)$
      ...
      \begin{equation*}
        ...
        ~~~
        P\left(T \geq t | \theta_L(t)\right) = \alpha_2.
      \end{equation*}
    \item If $F_T(t | \theta)$ is an increasing function of $\theta$ for each $t$, define $\theta_L(t)$ and $\theta_U(t)$
      by
      \begin{equation*}
        ...
        ~~~
        P\left(T \leq t | \theta_L(t)\right) = \alpha_2.
      \end{equation*}
  \end{enumerate}
  Then the random interval $[\theta_L(T),  \theta_U(T)]$ is a $1 - \alpha$ confidence interval for $\theta$.
\end{theorem}
...
\begin{definition}{10.1.1}[Consistent sequence of estimators]
  A sequence of estimators $W_n = W_n(X_1, \ldots, X_n)$ is a \textit{consistent sequence of estimators}
...
\begin{theorem}{10.1.5}
  ...
  Let $a_1, a_2, \ldots$ and $b_1, b_2, \ldots$ be sequences of constants satisfying
  \begin{enumerate}[label=\roman*.]
...
\begin{theorem}{10.1.6}[Consistency of MLEs]
  Let \\ $X_1, X_2, \ldots,$ be iid $f(x|\theta)$, and let $L(\theta | \vec{x}) = \prod_{i=1}^{n} f(x_i | \theta)$
  be the likelihood function. Let $\hat{\theta}$ denote thee MLE of $\theta$. Let $\tau(\theta)$ be a
  continuous function of $\theta$. Under the regularity conditions of Miscellanea 10.6.2 on $f(x | \theta)$
...
\begin{theorem}{10.1.12}[Asymptotic efficiency of MLEs]
  Let $X_1, X_2, \ldots,$ be iid $f(x|\theta)$, let $\hat{\theta}$ denote the MLE of $\theta$, and let
  $\tau(\theta)$ be a continuous function of $\theta$. Under the regularity conditions of Miscellanea
